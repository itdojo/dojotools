...
from dojoutils.rootcheck import check_root
...
DEFAULT_WIFI_CHANNELS = WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"]
...
SUPPORTED_WIFI_ADAPTERS = {
    ...
    "awus036achm": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"],
    "awus036ach": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"],
    "awus1900": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"],
    "awus036ac": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"],
    "aws036acm": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"],
    "awus036acs": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"],
    "awus036nha": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"],
    "awpcie-1900u": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"],
    "awmc7615p": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"],
    "awus036axer": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"],
    "awus036nh": WIFI_CHANNELS["2.4GHz"],
    "awmc9886ph": WIFI_CHANNELS["5GHz"],
    "awus036ax": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"] + WIFI_CHANNELS["6GHz"],
    "awus036axm": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"] + WIFI_CHANNELS["6GHz"],
    "awus036axml": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"] + WIFI_CHANNELS["6GHz"],
}
...
def supported_adapters():
    ...
    """
    return [adapter for adapter in SUPPORTED_WIFI_ADAPTERS.keys()]
...
def get_channel_list(channel_selection):
    ...
    channel_mapping = {
        "2.4GHz": WIFI_CHANNELS["2.4GHz"],   # Channels 1-11
        "2.4GHz-all": WIFI_CHANNELS["2.4GHz"] + ["12", "13"],  # All 2.4GHz channels
        "5GHz": WIFI_CHANNELS["5GHz"],  # All 5GHz channels (20MHz primary channels)
        "5GHz-UNII1": ["36", "40", "44", "48"],  # 5GHz Sub-bands
        "5GHz-UNII2": ["52", "56", "60", "64"],  # 5GHz Sub-bands
        "5GHz-UNII3": ["149", "153", "157", "161", "165"],  # 5GHz Sub-bands
        "6GHz": WIFI_CHANNELS["6GHz"],  # All 6GHz channels (20MHz primary channels)
        "all": WIFI_CHANNELS["2.4GHz"] + WIFI_CHANNELS["5GHz"] + WIFI_CHANNELS["6GHz"]
    }
    return channel_mapping.get(channel_selection, DEFAULT_WIFI_CHANNELS)  # Default to 2.4 & 5GHz


def hopper(iface, dwell=0.15, channels=DEFAULT_WIFI_CHANNELS, adapter=None, mode="random", stop_event=None):
    """
    ...
    from dojoutils.channelhopper import get_channel_list, SUPPORTED_WIFI_ADAPTERS, DEFAULT_WIFI_CHANNELS  # Ensure import works
    
    # Determine the channel list
    if adapter:
        channels = SUPPORTED_WIFI_ADAPTERS.get(adapter, DEFAULT_WIFI_CHANNELS)
    else:
...
if __name__ == "__main__":
    ...
    channels = input("Channel selection (2.4GHz, 5GHz, 6GHz, all, or custom list): ") or DEFAULT_WIFI_CHANNELS
